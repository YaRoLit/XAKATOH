...
def read_checks(input_path, output_path, files):
    ...
    for file in files:
        ...
        boxes, words = calculate_boxes(result)
        chk = ' '.join(words)     

        box_name = output_path + '/box/' + file.split('.')[0] + '.txt'
        save_box(box_name, result)
        ...
        save_json(json_name, model.find_entities(chk, boxes, words))


def calculate_boxes(result):
    '''
    Функция возвращает координаты распознанных текстовых блоков (правый верхний
    и левый нижний углы), а также список всех слов (это для оракула надо).
    '''
    boxes = []
    words = []
    for idx in range(len(result)):
        res = result[idx] # Это распознанные текстовые блоки с их 
        for line in res:
            x1 = int(line[0][0][0])
            y1 = int(line[0][0][1])
            x2 = int(line[0][2][0])
...
def save_box(filename, result):
    ...
    with open(filename, 'w') as f:
        for idx in range(len(result)):
            ...
            for line in res:
                coordinates = ','.join([str(int(j)) for i in line[0] for j in i])
                text = str(line[-1][0])
                f.write(coordinates + ',' + text + '\n')
